...
if st.button("Рассчитать риск"):
    if -1 in form_data.values():
        ...
        if total_score <= 3:
            st.write("Низкий риск ССЗ. Продолжайте вести здоровый образ жизни!")
        elif total_score <= 9:
            st.write("Средний риск ССЗ. Вам стоит пересмотреть свой образ жизни и обратиться за консультацией к врачу-терапевту.")
        else:
            st.write("Высокий риск ССЗ. Вам следует как можно скорее обратиться за консультацией к врачу-терапевту!")